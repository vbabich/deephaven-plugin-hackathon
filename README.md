...
Within the `src` directory, the deephaven_plugin_hackathon directory contains the Python code, and the `js` directory contains the JavaScript code.

The Python files have the following structure:
`deephaven_plugin_hackathon_object.py` defines a simple Python class that can send messages to the client.
`deephaven_plugin_hackathon_type.py` defines the Python type for the plugin (which is used for registration) and a simple message stream.
...
`DeephavenPluginHackathonPlugin.ts` registers the plugin with Deephaven.
`DeephavenPluginHackathonView.tsx` defines the plugin panel and message handling.
